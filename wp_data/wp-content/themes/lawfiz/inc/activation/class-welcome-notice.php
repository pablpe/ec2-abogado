...
class Lawfiz_Welcome_Notice {
	...
	*/
	public function __construct() {
		...
		// Render Notice
		add_action( 'admin_notices', [$this, 'lawfiz_render_notice'] );

		// Enque AJAX Script
		add_action( 'admin_enqueue_scripts', [$this, 'lawfiz_admin_enqueue_scripts'], 5 );

		// Dismiss
		add_action( 'admin_enqueue_scripts', [$this, 'lawfiz_notice_enqueue_scripts'], 5 );
		add_action( 'wp_ajax_lawfiz_dismissed_handler', [$this, 'lawfiz_dismissed_handler'] );

		// Reset
		add_action( 'switch_theme', [$this, 'lawfiz_reset_notices'] );
		add_action( 'after_switch_theme', [$this, 'lawfiz_reset_notices'] );

		// Install Plugins
		add_action( 'wp_ajax_lawfiz_install_activate_spiraclethemes_site_library', [$this, 'lawfiz_install_activate_spiraclethemes_site_library'] );
		add_action( 'wp_ajax_nopriv_lawfiz_install_activate_spiraclethemes_site_library', [$this, 'lawfiz_install_activate_spiraclethemes_site_library'] );
	...
	** Get plugin status.
	*/
	public function lawfiz_get_plugin_status( $plugin_path ) {
	...
	** Install a plugin.
	*/
	public function lawfiz_install_plugin( $plugin_slug ) {
		if ( ! current_user_can( 'install_plugins' ) ) {
			return;
		}
	...
	** Update a plugin.
	*/
	public function lawfiz_update_plugin( $plugin_path ) {
		if ( ! current_user_can( 'install_plugins' ) ) {
			return;
		}
	...
	** Update all plugins.
	*/
	public function lawfiz_update_all_plugins() {
		if ( ! current_user_can( 'install_plugins' ) ) {
			return;
		}
	...
	** Activate a plugin.
	*/
	public function lawfiz_activate_plugin( $plugin_path ) {
	...
	*/
	public function lawfiz_install_activate_spiraclethemes_site_library() {
		...
		$plugin_status = $this->lawfiz_get_plugin_status( 'spiraclethemes-site-library/spiraclethemes-site-library.php' );

		if ( 'not_installed' === $plugin_status ) {
			$this->lawfiz_install_plugin( 'spiraclethemes-site-library' );
			$this->lawfiz_activate_plugin( 'spiraclethemes-site-library/spiraclethemes-site-library.php' );

		} else {
			if ( 'inactive' === $plugin_status ) {
				$this->lawfiz_activate_plugin( 'spiraclethemes-site-library/spiraclethemes-site-library.php' );
			} elseif ( 'inactive_update' === $plugin_status || 'active_update' === $plugin_status ) {
				$this->lawfiz_update_plugin( 'spiraclethemes-site-library/spiraclethemes-site-library.php' );
				$this->lawfiz_activate_plugin( 'spiraclethemes-site-library/spiraclethemes-site-library.php' );
			}
		}

		if ( 'active' === $this->lawfiz_get_plugin_status( 'spiraclethemes-site-library/spiraclethemes-site-library.php' ) ) {
	...
	** Render Notice
	*/
	public function lawfiz_render_notice() {
		global $pagenow;
		...
		if ( 'spiraclethemes-site-library' !== $screen->parent_base ) {
			...
			if ( ! get_transient( $transient_name ) ) {
				?>
				<div class="lawfiz-notice notice notice-info is-dismissible" data-notice="<?php echo esc_attr( $transient_name ); ?>">
					<button type="button" class="notice-dismiss"></button>

					<?php $this->lawfiz_render_notice_content(); ?>
				</div>
				<?php
			}
		}
	...
	*/
	public function lawfiz_render_notice_content() {
		$action = 'install-activate';
		$redirect_url = 'admin.php?page=one-click-demo-import';
		$sslibrary_status = $this->lawfiz_get_plugin_status( 'spiraclethemes-site-library/spiraclethemes-site-library.php' );
	...
	** Reset Notice.
	*/
	public function lawfiz_reset_notices() {
	...
	** Dismissed handler
	*/
	public function lawfiz_dismissed_handler() {
	...
	*/
	public function lawfiz_notice_enqueue_scripts( $page ) {
	...
	*/
	public function lawfiz_admin_enqueue_scripts( $page ) {
				// Enqueue Scripts
		wp_enqueue_script( 'lawfiz-welcome-notic-js', get_template_directory_uri() . '/inc/activation/js/welcome-notice.js', ['jquery'], false, true );

		wp_localize_script( 'lawfiz-welcome-notic-js', 'lawfiz_localize', [
		...
		// Enqueue Styles.
		wp_enqueue_style( 'lawfiz-welcome-notic-css', get_template_directory_uri() . '/inc/activation/css/welcome-notice.css' );
	}
